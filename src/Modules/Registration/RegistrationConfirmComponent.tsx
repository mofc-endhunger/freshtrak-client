...
const RegistrationConfirmComponent: React.FC<RegistrationConfirmProps> = (props) => {
  ...
  /**
   * The reserved appointment window, as passed by whichever flow navigated here.
   * The form-based flow (RegistrationContainer) sends `eventTimeStamp`; the
   * household quick-confirm flow (EventSlotsModalComponent) sends `event_slot`.
   * Accept either so the specific window is shown instead of the overall event
   * hours, and so a future navigator using either key keeps working.
   */
  const reservedSlot: ReservedSlot | undefined =
    location.state?.eventTimeStamp ?? location.state?.event_slot;
  const event_slot_id = reservedSlot?.event_slot_id;
  ...
  const [showGuestSigninModal, setShowGuestSigninModal] = useState<boolean>(false);
  const [showRegisterAnotherDialog, setShowRegisterAnotherDialog] = useState<boolean>(false);
  ...
  // Prefer the id handed over by the navigating flow; fall back to session storage
  // for flows that only persist it there.
  const eventDateId: string | null =
    location.state?.eventDateId ?? StorageService.getRegisteredEventDateID();
  ...
  useEffect(fetchBusinesses, [getEvent, isError, pageError, selectedEvent, user, userToken]);
  ...
  const family_member_count = adults_in_household + children_in_household + seniors_in_household;
  ...
  const eventTime =
    reservedSlot?.start_time && reservedSlot?.end_time
      ? `${reservedSlot.start_time} - ${reservedSlot.end_time}`
      : `${event.startTime} - ${event.endTime}`;

  // Format agency address
  const agencyAddress = [eventAddress, eventCity, eventState, eventZip].filter(Boolean).join(', ');
  ...
  return (
    <Fragment>
      ...
      {event && isCaseManager && (
        <div className="mt-20 max-w-6xl mx-auto px-4 pb-24">
          <h1 className="text-3xl font-bold text-center mt-5 mb-8">
            {localization.cm_registration_complete || 'Registration Complete!'}
          </h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            <Card>
              <CardContent className="py-6">
                <h2 className="text-xl font-bold text-gray-900 mb-1">{event.agencyName}</h2>
                <p className="text-lg text-gray-700 mb-3">{(event as any).eventName || ''}</p>
                ...
                {address_line_1 && (
                  <div className="flex items-center space-x-2">
                    ...
                    <p className="text-sm text-gray-600">{address_line_1},</p>
                    <p className="text-sm text-gray-600">
                      {[city, state].filter(Boolean).join(', ')} {zip_code}
                    </p>
                  </div>
                )}
                {phone && (
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <p className="text-sm text-gray-600">{user_data?.email || ''}</p>
                </div>
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <p className="text-sm text-gray-600 font-semibold">
                    ...
                    {localization.family_member_count_plural || 'family members'}
                  </p>
                </div>
              </CardContent>
            </Card>
                ...
                <div className="flex justify-center p-4 bg-white">
                  <QRCode
                    value={`https://secure.pantrytrak.com/mobile/qr_code_processing.php?code=${identification_code.toUpperCase()}&event_date_id=${eventDateId}${
                      event_slot_id ? '&event_slot_id=' + event_slot_id : ''
                    }`}
                  />
                </div>
              </CardContent>
          ...
          <div className="flex flex-col gap-4 w-full items-center justify-center">
            {location.state?.eventDateId && (
              <Button
                ...
                onClick={() => setShowRegisterAnotherDialog(true)}
              >
          ...
          <Dialog open={showRegisterAnotherDialog} onOpenChange={setShowRegisterAnotherDialog}>
            <DialogContent className="sm:max-w-md bg-white border border-gray-200 text-gray-900">
              <DialogHeader>
                <DialogTitle className="text-center text-gray-900">
                  {localization.cm_save_before_leaving_title || 'Save Confirmation?'}
                </DialogTitle>
              ...
              <div className="flex gap-3 mt-4 justify-end">
                <Button variant="outline" onClick={() => setShowRegisterAnotherDialog(false)}>
        ...
        <div className="mt-20 max-w-6xl mx-auto px-4">
          <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-24 register-confirmation">
            ...
            <div className="address-wrap mb-5">
              <div className="date-wrapper">{formatDateDayAndDate(event.date)}</div>
              ...
              <>
                ...
                <p
                  ...
                  dangerouslySetInnerHTML={{
                    __html: sanitizeHtml(event.eventDetails),
                  }}
                />
              </>
            )}
      ...
      {/* Guest Sign-in Modal */}
      <Dialog open={showGuestSigninModal} onOpenChange={handleGuestModalOpenChange}>
        <DialogContent className="sm:max-w-md bg-white border border-gray-200 text-gray-900">
          <DialogHeader>
            <DialogTitle className="text-center text-gray-900">
