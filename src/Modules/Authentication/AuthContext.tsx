    ...
    try {
      ...
      try {
        ...
        if (!result.isSignedIn) {
          ...
          console.warn('Sign-in returned but isSignedIn is false:', result);
          ...
          const needsConfirmation =
            ...
            (signInStep && String(signInStep).includes('CONFIRM')) ||
        ...
        const errorName =
          ...
          '';
        const errorCode =
          signInError?.code || signInError?.cause?.code || signInError?.underlyingError?.code || '';
        ...
        const isUnconfirmedUserError =
          ...
          errorMessage.toLowerCase().includes('user needs to be confirmed') ||
          ...
          httpStatus === 400 ||
          (errorMessage.includes('400') && errorMessage.toLowerCase().includes('bad request'));
        ...
        if (isUnconfirmedUserError) {
          const unconfirmedError: any = new Error(errorMessage || 'User account is not confirmed');
          unconfirmedError.name = errorName || 'UserNotConfirmedException';
      ...
      if (result.isSignedIn) {
        try {
          ...
          console.warn('Could not fetch session tokens:', sessionError);
        }
      }
      ...
      if (result.isSignedIn) {
        ...
        const storedUserName = StorageService.getItem<string>(userNameKey);
        ...
        if (userName === email && userAttributes?.name && userAttributes.name.trim() !== '') {
        ...
        let accountCreatedDate = userAttributes?.account_created_date || null;
        let accountLastModified = userAttributes?.account_last_modified || null;
          ...
          try {
            ...
            if (tokenParts.length === 3) {
              ...
              if (payload.auth_time && !accountCreatedDate) {
                accountCreatedDate = new Date(payload.auth_time * 1000).toISOString();
              }
              // iat is "issued at" time - last auth time
              if (payload.iat && !accountLastModified) {
                accountLastModified = new Date(payload.iat * 1000).toISOString();
              }
            }
          } catch (jwtError) {
            console.warn('Could not extract dates from JWT token:', jwtError);
          }
        }
        ...
        if (accessToken) {
          ...
          console.warn('⚠️ AuthContext - No accessToken available to set userToken');
        }
      ...
      if (isUnconfirmedUserError) {
        const unconfirmedError: any = new Error(error.message || 'User account is not confirmed');
        unconfirmedError.name = error.name || error.__type || 'UserNotConfirmedException';
    ...
    try {
      ...
      if (password) {
        try {
          ...
          console.warn('Could not automatically sign in user:', signInError);
