...
import { createUserRecordSingleAttempt, isNotFoundError } from '../../Utils/UserRecordHelper';
    ...
    const fetchHouseholdData = async () => {
      ...
      try {
        ...
        const hasMinimalData =
          !userInfo?.address_line_1 || !userInfo?.city || !userInfo?.state || !userInfo?.zip_code;

        if (hasMinimalData) {
          setShowSkippedSetupPrompt(true);
        }
      } catch (error: any) {
        console.warn('AccountPage: Error fetching household data:', error);

        // Check if this is a "User not found" / 404 error
        if (isNotFoundError(error)) {
          const result = await createUserRecordSingleAttempt(householdsApiService, user?.name);
          ...
          if (result.success) {
            ...
            try {
              ...
              console.error('Failed to fetch user data after fallback creation:', retryError);
            }
          }
        }
          ...
          <div className="mb-8">
            ...
            <div className="flex flex-col items-center py-8">
              ...
              <p className="font-noto-sans font-normal text-base leading-[22px] tracking-normal text-center text-gray-500 mt-1">
                ...
                householdData.members[0].is_head_of_household === 1
                  ? localization.text_head_of_household || 'Head of Household'
                  : 'Household Member'}
              </p>
            </div>
          </div>
            ...
            <TabsContent value="account" className="mt-6">
                      ...
                      <div className="flex items-start space-x-4">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex space-x-3">
                            <Button
                              onClick={() => {
                                setShowSkippedSetupPrompt(false);
                                navigateToHouseholdSetup();
                              }}
                              className="bg-orange-600 hover:bg-orange-700 text-white"
                            >
                              {localization.button_set_up_household || 'Set Up Household'}
                            </Button>
                            <Button
                              onClick={() => setShowSkippedSetupPrompt(false)}
                              variant="ghost"
              ...
              >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  <AccountInfoSection householdData={householdData} />
