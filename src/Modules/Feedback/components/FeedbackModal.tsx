...
const AlreadySubmittedContent: React.FC<{ onClose: () => void }> = ({ onClose }) => (
  <div className="p-5 space-y-4">
    <div className="text-center">
      <p className="text-gray-700 mb-4">{localization.feedback_already_submitted_message}</p>
    </div>
...
const FeedbackFormContent: React.FC<{
  ...
  const overallRatingQuestion = (
    localization.feedback_visit_question || 'How was your visit on {date} to {location}?'
  )
    ...
    <form onSubmit={handleSubmit} className="p-5 space-y-5 bg-white">
      ...
      <div>
        <label className="text-sm font-semibold text-gray-800 mb-2 block">
          ...
          <span className="text-gray-400 font-normal ml-1">{localization.label_optional}</span>
        </label>
        <Textarea
          ...
          maxLength={1000}
        />
        <p className="text-xs text-gray-400 mt-1 text-right">{formState.comments.length}/1000</p>
      </div>
...
const FeedbackModal: React.FC<FeedbackModalInternalProps> = ({
  ...
  const renderContent = () => {
    switch (modalState) {
      ...
      case 'error':
        return (
          <ErrorContent error={error || localization.feedback_error_generic} onRetry={reload} />
        );
