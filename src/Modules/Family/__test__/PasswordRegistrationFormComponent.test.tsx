...
describe('PasswordRegistrationFormComponent', () => {
  ...
  describe('Initial Rendering', () => {
    it('renders the main heading', () => {
      ...
      expect(screen.getByText('Create FreshTrak Account')).toBeInTheDocument();
    });

    it('renders the description text', () => {
      renderPasswordRegistrationForm();
      expect(
        screen.getByText(/Input a password to create a FreshTrak account/),
      ).toBeInTheDocument();
    });
    ...
    it('renders Google email link', () => {
      ...
      expect(emailLink).toHaveAttribute('href', 'https://support.google.com/mail/answer/56256');
  ...
  describe('Form Validation', () => {
    ...
    it('validates password confirmation matches', () => {
      ...
      const lastCall = mockRegister.mock.calls.find((call) => call[0] === 'password_confirm');
      const validationOptions = lastCall[1];
      const matchesPasswordValidator = validationOptions.validate.matchesPassword;

      // Test with matching password
      expect(matchesPasswordValidator('testpassword123')).toBe(true);

      // Test with non-matching password
      expect(matchesPasswordValidator('differentpassword')).toBe('Passwords should match!');
    });
  });
  ...
  describe('Input Attributes and Accessibility', () => {
    ...
    it('has proper labels associated with inputs', () => {
      ...
      expect(confirmPasswordInput).toHaveAttribute('id', 'password_confirm');
    });

    it('has proper form structure', () => {
      renderPasswordRegistrationForm();

      const form = screen.getByText('Create FreshTrak Account').closest('div');
  ...
  describe('Responsive Design', () => {
    it('applies responsive text alignment classes', () => {
      ...
      const headerContainer = screen.getByText('Create FreshTrak Account').closest('div');
    ...
    it('applies responsive margin classes', () => {
      ...
      const descriptionContainer = screen.getByText(/Input a password/).closest('p');
      expect(descriptionContainer).toHaveClass('max-w-md', 'mx-auto', 'md:mx-0');
    });
    ...
    it('applies consistent spacing classes', () => {
      ...
      const formFieldsContainer = screen.getByLabelText('Email Address').closest('div');
    ...
    it('applies responsive spacing between elements', () => {
      ...
      const emailField = screen.getByLabelText('Email Address').closest('div');
  ...
  describe('Component Integration', () => {
    ...
    it('integrates with react-hook-form register function', () => {
      ...
      expect(mockRegister).toHaveBeenCalledWith('password_confirm', expect.any(Object));
    });
  ...
  describe('Styling and Visual Elements', () => {
    ...
    it('applies proper font weights', () => {
      ...
      const heading = screen.getByText('Create FreshTrak Account');
      const labels = screen.getAllByText(/Email Address|Password|Confirm Password/);
