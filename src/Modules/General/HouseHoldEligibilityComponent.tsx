...
import { HouseHoldEligibilityComponentProps, ParsedBodyData } from './types/eligibility.types';
 ...
 */
const HouseHoldEligibilityComponent: React.FC<HouseHoldEligibilityComponentProps> = ({
  header = '',
  body = '',
  footer = '',
}) => {
  ...
  if (body) {
    try {
      ...
      columnData = parsed.columns && Array.isArray(parsed.columns) ? parsed.columns : [];
      rowsData = parsed.rows && Array.isArray(parsed.rows) ? parsed.rows : [];
