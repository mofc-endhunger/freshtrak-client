...
const transformHouseholdDataToUserData = (householdData: UsersMeResponse | null) => {
  if (!householdData) {
    return {
      first_name: '',
      last_name: '',
  ...
  const headOfHousehold =
    householdData.members?.find((member) => member.is_head_of_household === 1) ||
  ...
  const last_name = headOfHousehold?.last_name || nameParts.slice(1).join(' ') || '';
...
export interface EventSlotsModalProps {
  ...
  onSlotChange: (event: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => void;
}
...
const EventSlotsModalComponent: React.FC<EventSlotsModalProps> = ({
  ...
  const [householdData, setHouseholdData] = useState<UsersMeResponse | null>(null);
  ...
  const findEventSlot = (event_slot_id: string) => {
    ...
    return event_slots.find((event_slot) => {
      return parseInt(event_slot_id) === parseInt(event_slot.event_slot_id);
    });
  };
  ...
  const handleSlotSelection = async (slotId: string) => {
    ...
    try {
      ...
      const hasCompletedSetup = Boolean(
        headOfHousehold?.date_of_birth && headOfHousehold.date_of_birth !== '1900-01-01',
      );
  ...
  const navigateToRegistration = (slot: EventSlot, householdData: UsersMeResponse | null) => {
    navigate(
      `${targetUrl || RENDER_URL.REGISTRATION_FORM_URL}/${eventDateId}/${slot.event_slot_id}`,
      {
        state: {
          event_slot: slot,
  ...
  const handleHouseholdConfirm = async () => {
    ...
    try {
      ...
      const hohDob =
        householdData?.members?.find((m) => m.is_head_of_household === 1)?.date_of_birth ??
        householdData?.members?.[0]?.date_of_birth;
      ...
      <Dialog open={show} onOpenChange={setShow}>
        ...
        >
          <DialogHeader>
            ...
            >
              <span className="pr-3">
                <img aria-hidden="true" alt="" src={alarmIcon} className="w-6 h-6" />
              </span>
              {localization.dialog_choose_time_slot_title || 'Choose Time Slot'}
            </DialogTitle>
          ...
          <div className="container py-4 overflow-y-auto max-h-[45vh]">
            {isLoading ? (
              ...
              (() => {
                ...
                return (
                  <fieldset role="radiogroup" aria-labelledby="timeslot-modal-title">
                    ...
                    {eventHour.map((item, index) =>
                      item.event_slots
                        ...
                        .map((e, i) => {
                          ...
                          return (
                            <div className="flex items-center p-2 space-x-2" key={index + '-' + i}>
                              <input
                                ...
                                checked={String(selectedSlotId) === String(e.event_slot_id)}
                              ...
                              <span id={`${radioId}-description`} className="sr-only">
                                {e.open_slots}{' '}
                                {e.open_slots !== 1
