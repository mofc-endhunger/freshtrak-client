...
const AccountInfoSection: React.FC<AccountInfoSectionProps> = ({
			...
			<>
				...
				{formattedDOB && (
					<div>
						...
						{formattedDOB} · {calculateAgeFromDOB(dob)}{" "}
						{localization.label_years_old}
					</div>
				)}
				<div className="capitalize">
					{gender || localization.text_not_available || "N/A"}
				</div>
			</>
		);
	};
	...
	return (
		<div className="space-y-4">
			...
			{/* Information Card */}
			<InfoCard
				title={localization.card_title_information || "Information"}
			>
				{getHeadOfHouseholdInfo()}
			</InfoCard>

			{/* Address Card */}
			<InfoCard title={localization.card_title_address || "Address"}>
				{formatAddress()}
			</InfoCard>

			{/* Contact Card */}
			<InfoCard title={localization.card_title_contact || "Contact"}>
				{formatContact()}
			</InfoCard>
		</div>
	);
