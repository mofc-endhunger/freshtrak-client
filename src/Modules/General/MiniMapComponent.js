...
const MiniMapComponent = ({
  ...
  useEffect(() => {
    ...
    const loadMap = async () => {
      ...
      if (latitude && longitude && isValidCoord(latitude) && isValidCoord(longitude)) {
        coords = { lat: latitude, lng: longitude };
      } else if (coordinates && isValidCoord(coordinates.lat) && isValidCoord(coordinates.lng)) {
        coords = coordinates;
      } else {
        try {
  ...
  return (
    ...
    >
      <div ref={mapRef} className="map-container" style={{ height: '100%', minHeight: 100 }} />
