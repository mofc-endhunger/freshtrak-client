...
export const HouseholdContainer: React.FC<HouseholdContainerProps> = ({ className = '' }) => {
  ...
  useEffect(() => {
    const loadHouseholdData = async () => {
      ...
      const isSetupRoute = window.location.pathname === RENDER_URL.HOUSEHOLD_SETUP_URL;
      if (isSetupRoute) {
        ...
        if (fromAccount) {
          ...
          try {
            ...
            const initialHouseholdData = {
              ...
              primary_last_name: user.name?.split(' ').slice(1).join(' ') || '',
            ...
            const response = await householdsApiService.createHousehold(initialHouseholdData);
            ...
            console.error('Error creating initial household:', error);
  ...
  const handleSetupComplete = async (registrationData: any) => {
    try {
      ...
      const langCode =
        registrationData.preferred_language || currentHouseholdData.preferred_language || 'en';
      ...
      const updateData = {
        ...
        members: (() => {
          ...
          const deletedIds = registrationData.deleted_member_ids || [];
          ...
          if (updatedMembers.length > 0) {
            ...
            if (registrationData.gender) {
              ...
              const normalizedGender = normalizeGender(registrationData.gender);
            ...
            updatedMembers[0] = {
              ...updatedMembers[0],
              first_name: registrationData.first_name || updatedMembers[0].first_name,
              last_name: registrationData.last_name || updatedMembers[0].last_name,
              middle_name: registrationData.middle_name || updatedMembers[0].middle_name,
              date_of_birth: registrationData.date_of_birth || updatedMembers[0].date_of_birth,
              gender_id:
                genderId !== null
                  ? genderId
          ...
          familyMembers.forEach((member: any) => {
            ...
            if (member.id && member.id > 0) {
              ...
              const existingIndex = updatedMembers.findIndex((m: any) => m.id === member.id);
              if (existingIndex !== -1) {
          ...
          const newMembers = familyMembers.filter((member: any) => !member.id || member.id < 0);
          const formattedNewMembers = newMembers.map((member: any) => ({
        ...
        counts: (() => {
          ...
          const seniors = Number(registrationData.seniors_in_household) || 0;
          const adults = Number(registrationData.adults_in_household) || 0;
          const children = Number(registrationData.children_in_household) || 0;
      ...
      await householdsApiService.updateHousehold(currentHouseholdData.added_by, updateData);
