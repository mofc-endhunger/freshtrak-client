...
export const isUserAlreadyExistsError = (error: any): boolean => {
  return error?.type === 'CONFLICT' || error?.message?.includes('already exists');
};
...
export const isAuthenticationError = (error: any): boolean => {
  return error?.response?.status === 401 || error?.type === 'AUTHENTICATION_ERROR';
};
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (isAuthenticationError(error)) {
        console.warn('Auth error during user creation, will retry on next page load');
        onError?.(error);
...
export const resolveUserNameWithFallback = async (
  ...
  const pendingNameIsForThisUser =
    pendingUser?.name &&
    (!pendingUser.email || !confirmedEmail || pendingUser.email === confirmedEmail);
...
export const createUserRecordAfterConfirmation = async (
  ...
  const {
    ...
    apiService,
    maxRetries = 3,
    guestToken: passedGuestToken,
  } = options;

  // Resolve user name with fallbacks
  const { userName, userEmail } = await resolveUserNameWithFallback(pendingEmail, authUser);
 ...
 * ```
 */
export const setNewUserSignupFlag = (email: string, userRecordCreated = false): void => {
 ...
 */
export const updateNewUserSignupFlag = (email: string, userRecordCreated: boolean): boolean => {
