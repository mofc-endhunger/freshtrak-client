...
jest.mock('../AddressComponent', () => ({
  ...
  default: ({ register, errors }: any) => (
    <div data-testid="address-component">
      ...
      />
      {errors.address_line_1 && <span data-testid="address-error">Address error</span>}
    </div>
  ),
}));
...
jest.mock('../PrimaryInfoFormComponent', () => ({
  ...
  default: ({ register, errors, getValues, control }: any) => (
    <div data-testid="primary-info-component" data-has-control={control ? 'true' : 'false'}>
      <input {...register('first_name')} data-testid="first-name" placeholder="First Name" />
      {errors.first_name && <span data-testid="first-name-error">First name error</span>}
    </div>
  ),
}));
  ...
  default: ({ register, errors, event }: any) => (
    <div data-testid="member-count-component">
      <input
        ...
        defaultValue="0"
      />
      {errors.seniors_in_household && <span data-testid="seniors-error">Seniors error</span>}
    </div>
  ),
}));
...
describe('EditFamilyContainer', () => {
  ...
  describe('Initial Rendering', () => {
    ...
    it('displays the description text', () => {
      renderEditFamilyContainer();
      expect(
        screen.getByText("Update your family's information and preferences"),
      ).toBeInTheDocument();
    });
  });
  ...
  describe('Form Functionality', () => {
    it('calls onSubmit when form is submitted', async () => {
      ...
      const form = screen.getByTestId('address-component').closest('form')!;
  ...
  describe('Responsive Design', () => {
    it('applies responsive container classes', async () => {
      ...
      const container = screen.getByText('Edit Family Information').closest('div')?.parentElement;
    ...
    it('applies responsive padding classes', async () => {
      ...
      const formContainer = screen.getByTestId('address-component').closest('form')?.parentElement;
