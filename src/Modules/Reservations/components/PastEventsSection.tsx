 ...
 */
const PastEventsSection: React.FC<PastEventsSectionProps> = ({ onEventClick }) => {
  ...
  const fetchPastReservations = useCallback(async () => {
    ...
    try {
      const data: ReservationsResponse = await reservationsApiService.getPastReservations();
      ...
      setError(localization.error_loading_reservations || 'Failed to load past events');
    } finally {
      setIsLoading(false);
    }
    ...
    <div className="w-full mt-8">
      ...
      <div className="mb-4">
        ...
        <p className="font-noto-sans text-sm text-gray-500 mt-1">
          {localization.text_past_events_description || 'Your event history from the last 30 days.'}
        </p>
      </div>
      ...
      <LoadingCard
        isLoading={isLoading}
        operation={localization.loading_reservations || 'Loading past events...'}
        className="w-full"
      >
        {/* Error State */}
        {error && (
        ...
        {!error && pastReservations.length === 0 && (
          <div className="text-center py-12 bg-gray-50 rounded-lg">
            ...
            <p className="font-noto-sans text-base text-gray-500">
              {localization.text_no_past_events || 'No past events found.'}
            </p>
          </div>
        )}
