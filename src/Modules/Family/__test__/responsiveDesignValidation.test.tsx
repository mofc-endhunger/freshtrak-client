...
describe('Responsive Design Validation', () => {
  ...
  describe('FamilyContainer Responsive Design', () => {
    ...
    test('should have responsive section classes', () => {
      ...
      expect(section).toHaveClass('container', 'mx-auto', 'px-4', 'py-8', 'max-w-4xl');
    });
    ...
    test('should have responsive form container', () => {
      ...
      const formContainer = screen.getByTestId('family-registration-form');
  ...
  describe('PrimaryInfoFormComponent Responsive Design', () => {
    test('should have responsive form layout', () => {
      ...
      const formComponent = screen.getByTestId('primary-info-form-component');
    ...
    test('should have responsive input fields', () => {
      ...
      expect(firstNameInput).toHaveClass('w-full', 'rounded-md', 'border');

      const lastNameInput = screen.getByTestId('last-name-input');
      expect(lastNameInput).toHaveClass('w-full', 'rounded-md', 'border');
    });
    ...
    test('should have responsive label styling', () => {
      ...
      expect(firstNameLabel).toHaveClass('text-sm', 'font-medium', 'text-gray-700');
    });
    ...
    test('should have responsive select styling', () => {
      ...
      expect(genderSelect).toHaveClass('w-full', 'rounded-md', 'border');
    });
  });
  ...
  describe('AddressComponent Responsive Design', () => {
    ...
    test('should have responsive input fields', () => {
      ...
      expect(addressInput).toHaveClass('w-full', 'rounded-md');

      const cityInput = screen.getByTestId('city-input');
      expect(cityInput).toHaveClass('w-full', 'rounded-md', 'border');

      const zipInput = screen.getByTestId('zip-code-input');
      expect(zipInput).toHaveClass('w-full', 'rounded-md', 'border');
    });
    ...
    test('should have responsive label styling', () => {
      ...
      expect(addressLabel).toHaveClass('text-sm', 'font-medium', 'text-gray-700');
    });
  ...
  describe('ContactInformationComponent Responsive Design', () => {
    test('should have responsive contact form layout', () => {
      ...
      const contactComponent = screen.getByTestId('contact-information-component');
    ...
    test('should have responsive input fields', () => {
      ...
      expect(phoneInput).toHaveClass('w-full', 'rounded-md', 'border');

      const emailInput = screen.getByTestId('email-input');
      expect(emailInput).toHaveClass('w-full', 'rounded-md', 'border');
    });
    ...
    test('should have responsive link styling', () => {
      ...
      expect(googleLink).toHaveClass('text-indigo-600', 'hover:text-indigo-500', 'underline');
    });
  });
  ...
  describe('Cross-Component Responsive Consistency', () => {
    test('should maintain consistent spacing across all components', () => {
      ...
      formSections.forEach((section) => {
        if (section.className.includes('font-semibold')) {
          ...
          const hasNeutral = section.className.includes('text-gray-900');
          const hasTheme = section.className.includes('text-text-color');
          const hasHighlight = section.className.includes('text-highlight');
    ...
    test('should maintain consistent label styling across all components', () => {
      ...
      labels.forEach((label) => {
        if (label.className.includes('text-sm')) {
    ...
    test('should maintain consistent button styling across all components', () => {
      ...
      buttons.forEach((button) => {
        if (button.textContent?.includes('Continue')) {
          ...
          const hasHighlightBg = button.className.includes('bg-highlight');
          const hasIndigoBg = button.className.includes('bg-indigo-600');
    ...
    test('should have mobile-friendly button sizing', () => {
      ...
      buttons.forEach((button) => {
        if (button.textContent?.includes('Continue')) {
          ...
          const hasPadding = hasPx4 || hasPx6 || button.className.includes('px-');
          const hasVerticalPadding = hasPy2 || hasPy3 || button.className.includes('py-');
    ...
    test('should maintain consistent focus states', () => {
      ...
      inputs.forEach((input) => {
        ...
        if (input.className.includes('focus:outline-none')) {
          expect(input).toHaveClass('focus:outline-none', 'focus:ring-2', 'focus:ring-indigo-500');
        }
      });
    });
  });
