...
interface LocalFoodBankComponentProps {
	zipCode?: string | null;
}
...
const LocalFoodBankComponent: React.FC<LocalFoodBankComponentProps> = props => {
	let [foodBankData, setFoodBankData] = useState<FoodBankData | string>({});
							...
							<div className="d-flex align-items-center">
								<span className="search-list-logo">
									<img
										alt="logo"
										src={
											(foodBankData as FoodBankData).logo
										}
									/>
								</span>
								<span className="font-weight-bold ml-2">
									{(foodBankData as FoodBankData).name}
								</span>
							</div>
						</div>
						<div className="col-lg-4 col-sm-6 caption-text">
							{(foodBankData as FoodBankData).address}{" "}
							{(foodBankData as FoodBankData).city},{" "}
							{(foodBankData as FoodBankData).state}{" "}
							{(foodBankData as FoodBankData).zip}
						</div>
						<div className="col-lg-4 col-sm-6 caption-text">
							<div>{(foodBankData as FoodBankData).phone}</div>
							<div className="link-wrap">
								<a
									href={
										(foodBankData as FoodBankData)
											.display_url
									}
									target="_blank"
									rel="noopener noreferrer"
								>
									{(foodBankData as FoodBankData).display_url}
								</a>
							</div>
						</div>
					</div>
