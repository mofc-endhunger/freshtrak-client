...
describe('HouseholdInfoDisplay', () => {
  ...
  describe('Rendering', () => {
    it('renders household information correctly', () => {
      ...
      expect(screen.getByTestId('household-name')).toHaveTextContent('Smith Family');
    ...
    it('renders without household name when name is empty', () => {
      ...
      expect(screen.queryByTestId('household-name')).not.toBeInTheDocument();
    });

    it('applies custom className', () => {
      const { container } = render(
        <HouseholdInfoDisplay householdData={mockHouseholdData} className="custom-class" />,
      );
  ...
  describe('Address Formatting', () => {
    ...
    it('shows "No address provided" when all address fields are missing', () => {
      ...
      expect(screen.getByTestId('household-address')).toHaveTextContent('No address provided');
    });
  });
  ...
  describe('Member Count Formatting', () => {
    ...
    it('handles singular member counts', () => {
      ...
      render(<HouseholdInfoDisplay householdData={dataWithSingularCounts} />);
    ...
    it('handles zero member counts', () => {
      ...
      expect(screen.getByTestId('household-members')).toHaveTextContent('No members');
      expect(screen.getByText(/Total Members/)).toBeInTheDocument();
    });
    ...
    it('handles only adults', () => {
      ...
      expect(screen.getByTestId('household-members')).toHaveTextContent('3 Adults');
    });
  });
  ...
  describe('Contact Information Formatting', () => {
    ...
    it('handles only phone number', () => {
      ...
      expect(screen.getByTestId('household-contact')).toHaveTextContent('Phone: 555-1234');
    });
    ...
    it('handles only email', () => {
      ...
      expect(screen.getByTestId('household-contact')).toHaveTextContent('Email: smith@example.com');
    });
    ...
    it('shows "No contact information" when both are missing', () => {
      ...
      expect(screen.getByTestId('household-contact')).toHaveTextContent('No contact information');
    });
  });
