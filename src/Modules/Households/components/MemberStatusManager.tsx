...
import { UserX, UserCheck, MoreVertical, AlertTriangle, CheckCircle, Clock } from 'lucide-react';
...
export const MemberStatusManager: React.FC<MemberStatusManagerProps> = ({
  ...
  const [pendingAction, setPendingAction] = useState<StatusChangeAction | null>(null);
  ...
  const handleStatusChange = async () => {
    ...
    try {
      ...
      onError?.(`Failed to ${pendingAction.action} member. Please try again.`);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="flex items-center space-x-2">
      {/* Status Indicator */}
      <div className={`px-2 py-1 rounded-full text-xs font-medium border ${getStatusBadgeColor()}`}>
      ...
      {variant === 'button' ? renderButtonVariant() : renderDropdownVariant()}
    </div>
  );
};
