...
jest.mock('../../Registration/components/HouseholdConfirmationModal', () => {
  return function MockHouseholdConfirmationModal({ isOpen, onConfirm, onReview }: any) {
    return isOpen ? (
...
jest.mock('../../Localization/LocalizationComponent', () => ({
  ...
  dialog_choose_time_slot_description: 'Select an available time slot for your registration.',
  button_go_back: 'Go Back',
  button_save_and_continue: 'Save and Continue',
  loading_loading: 'Loading...',
  sr_available_time_slots_registration: 'Available time slots for registration',
  text_slot: 'slot',
  text_slots: 'slots',
  text_available: 'available',
  aria_loading_time_slots: 'Loading time slots',
  sr_return_previous_without_slot: 'Return to previous page without selecting a time slot.',
  sr_proceed_registration_selected_slot: 'Proceed with registration using the selected time slot.',
}));
...
describe('EventSlotsModalComponent', () => {
  ...
  describe('API Integration', () => {
    test('should call API to get event hours', async () => {
      renderComponent();
      await waitFor(() => {
        expect(mockedAxios.get).toHaveBeenCalledWith(expect.stringContaining('/123/event_hours'));
      });
    });
  ...
  describe('User Interactions', () => {
    ...
    test('should check radio button when selectedSlotId matches', async () => {
      ...
      await waitFor(() => {
        const radioButton = screen.getByDisplayValue('1') as HTMLInputElement;
    ...
    test('should close modal when Save and Continue button is clicked', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId('household-confirmation-modal')).toBeInTheDocument();
      });
    });
  });
  ...
  describe('Household Confirmation Flow', () => {
    ...
    // SUP-559: the confirmation page reads the reserved window from `eventTimeStamp`.
    // Omitting it made registered users see the overall event hours instead of their
    // 10-minute appointment, and dropped event_slot_id from the check-in QR code.
    test('should navigate to confirmation with the reserved appointment window', async () => {
      renderComponent({ selectedSlotId: '1' });
      ...
      await waitFor(() => {
        expect(mockNavigate).toHaveBeenCalledWith(
          RENDER_URL.REGISTRATION_CONFIRM_URL,
    ...
    test('should persist the event date id for the confirmation QR code', async () => {
      renderComponent({ selectedSlotId: '1' });
      ...
      await waitFor(() => {
        expect(StorageService.getRegisteredEventDateID()).toBe('123');
      });
    });
    ...
    test('should have proper form structure', async () => {
      ...
      await waitFor(() => {
        ...
        radioButtons.forEach((radio, index) => {
          const label = screen.getByText(index === 0 ? '09:00 - 10:00' : '11:00 - 12:00');
  ...
  describe('Styling and Classes', () => {
    ...
    test('should apply proper radio button styling', async () => {
      ...
      await waitFor(() => {
        ...
        expect(radioButton).toHaveClass('h-4', 'w-4', 'text-indigo-600');
      });
    });
