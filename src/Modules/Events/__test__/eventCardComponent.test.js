...
import { render, fireEvent, screen } from '../../../Testing/mock-provider';
import EventCardComponent from '../EventCardComponent';
import { preformattedEventData } from '../../../Testing';

// Mock useAuth so FavoriteButton renders without AuthProvider
...
describe('FavoriteButton inside EventCardComponent', () => {
  test('renders the favorite button in tile variant', () => {
    render(
      <MemoryRouter>
        <EventCardComponent event={preformattedEventData} variant="tile" />
      </MemoryRouter>,
    );
    expect(screen.getByTestId('favorite-button')).toBeInTheDocument();
  });

  test('renders the favorite button in list variant', () => {
    render(
      <MemoryRouter>
        <EventCardComponent event={preformattedEventData} variant="list" />
      </MemoryRouter>,
    );
    expect(screen.getByTestId('favorite-button')).toBeInTheDocument();
  });

  test('clicking the star does not trigger card navigation', () => {
    ...
    // Should not throw or navigate — stopPropagation prevents the card click
