...
const ResourceListComponent: React.FC<ResourceListComponentProps> = ({ dataToChild }) => {
  ...
  const foodBankDisplay = (): string => {
    switch (foodBankArray.length) {
      ...
      case 1:
        return localization.text_food_bank_serving_zip_code.replace('{zip}', searchedZip || '');
      default:
        return localization.text_food_banks_serving_zip_code.replace('{zip}', searchedZip || '');
    }
  };
  ...
  return (
    <section className="py-5 mt-2.5 text-xs" aria-live="polite">
      <div className="text-[#495057] font-bold text-sm uppercase mb-4">{foodBankDisplay()}</div>
      {foodBankArray.map((value, index) => {
        const {
          foodbank: { name, address, city, state, zip, phone, display_url, logo, foodbank_texts },
        } = value;
        return (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 items-center mt-2">
              <div className="lg:col-span-1 sm:col-span-1">
                <div className="flex items-center">
                  <span className="h-8">
                    <img alt={localization.alt_logo || 'logo'} src={logo} className="max-h-full" />
                  </span>
                  <span className="font-bold ml-2 text-sm">{name}</span>
                </div>
              </div>
