...
describe('HouseholdInfoDisplay Accessibility', () => {
  describe('ARIA Attributes and Roles', () => {
    it('should have proper region role and ARIA attributes', () => {
      ...
      expect(mainRegion).toHaveAttribute('aria-labelledby', 'household-info-heading');
    });
    ...
    it('should have proper heading hierarchy', () => {
      ...
      expect(householdNameHeading).toHaveAttribute('id', 'household-name-heading');
    ...
    it('should have proper section elements with ARIA labels', () => {
      ...
      const addressSection = screen.getByText('Address').closest('section');
      const membersH4 = screen.getAllByText('Household Members').find((el) => el.tagName === 'H4');
      const membersSection = membersH4?.closest('section') ?? null;
      const contactSection = screen.getByText('Contact Information').closest('section');

      expect(addressSection).toHaveAttribute('aria-labelledby', 'address-heading');
      expect(membersSection).toHaveAttribute('aria-labelledby', 'members-heading');
  ...
  describe('Content Accessibility', () => {
    it('should have proper aria-describedby attributes for content', () => {
      ...
      expect(addressText).toHaveAttribute('aria-describedby', 'address-heading');
      expect(membersText).toHaveAttribute('aria-describedby', 'members-heading');
      expect(contactText).toHaveAttribute('aria-describedby', 'contact-heading');
    });
  ...
  describe('Data Formatting Accessibility', () => {
    ...
    it('should handle plural forms correctly', () => {
      ...
      const membersText = screen.getByTestId('household-members');
      expect(membersText).toHaveTextContent('2 Adults, 3 Children, 2 Seniors');
    });

    it('should format address information clearly', () => {
      render(<HouseholdInfoDisplay {...defaultProps} />);

      const addressText = screen.getByText('123 Test Street, Apt 4B, Test City, TS, 12345');
      expect(addressText).toBeInTheDocument();
    });
    ...
    it('should format contact information clearly', () => {
      ...
      const contactText = screen.getByText('Phone: 555-123-4567 • Email: test@example.com');
      expect(contactText).toBeInTheDocument();
    });
  });
