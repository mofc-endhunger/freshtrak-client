...
const HomeContainer: React.FC<HomeContainerProps> = () => {
  ...
  const getUsersReservations = async (): Promise<void> => {
    ...
    try {
      const response = await axios.get<UserReservationsApiResponse>(CREATE_RESERVATION, {
        headers: { Authorization: `Bearer ${userToken}` },
      });
      // Handle both direct array response and wrapped response
      const userRegData = Array.isArray(response.data) ? response.data : response.data?.data || [];
  ...
  const getEventByDateId = async (userRegData: UserRegistration[]): Promise<void> => {
    if (!userRegData || !Array.isArray(userRegData) || userRegData.length === 0) {
      setReservedEvents([]);
      return;
    }
  ...
  const getEvents = async (zip: string): Promise<void> => {
    if (zip) {
      setLoading(true);
      try {
        const resp: { data: EventsListApiResponse } = await axios.get(API_URL.EVENTS_LIST, {
          params: { zip_code: zip },
        });
  ...
  const EventList: React.FC<EventListProps> = (props) => {
    const filterEvents = (eventList: FilteredEvents): FilteredEvents => {
      ...
      if (props.filter === 'week') {
        ...
        const weekevents = entries.reduce((acc: FilteredEvents, item: [string, any]) => {
    ...
    if (agencyResponse) {
      let agencyDataSorted: FilteredEvents = EventHandler(agencyData) as FilteredEvents;
          ...
          <div className="text-left">
            <form onSubmit={handleSubmit(onSubmit)}>
              <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-end">
                ...
                <Button type="submit" className="px-4 py-2 w-full sm:w-auto" disabled={loading}>
                  {loading ? (
