 ...
 */
const SUPPORTED_LANGUAGES = ['en', 'spa', 'som', 'rus', 'tur', 'ara', 'zho', 'hin', 'nep'] as const;
 ...
 * ```
 */
export function validateLocalization(currentLanguage: SupportedLanguage = 'en'): ValidationResult {
 ...
 * ```
 */
export function checkKeyInAllLanguages(key: string): Record<SupportedLanguage, boolean> {
  const status: Record<SupportedLanguage, boolean> = {} as Record<SupportedLanguage, boolean>;
  const originalLanguage = localization.getLanguage?.() || 'en';
...
export function getLocalizationStats(): {
  ...
  const coveragePercentage =
    totalPossibleKeys > 0 ? Math.round((totalPresentKeys / totalPossibleKeys) * 100) : 100;
  ...
  SUPPORTED_LANGUAGES.forEach((language) => {
    ...
    coverageByLanguage[language] =
      result.totalKeys > 0 ? Math.round((present / result.totalKeys) * 100) : 100;
  });
