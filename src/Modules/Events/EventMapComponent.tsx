...
const isValidCoord = (val: number | string | undefined | null): boolean => {
  if (val === undefined || val === null || val === '' || val === '0.0' || val === '0') return false;
...
// Create orange dot icon for external agency markers
const createExternalAgencyIcon = (): L.DivIcon => {
  return L.divIcon({
    className: 'external-agency-marker',
    html: `<div class="external-marker-dot"></div>`,
    iconSize: [24, 24],
    iconAnchor: [12, 12],
  });
};
...
interface EventMapComponentProps {
  ...
  onExternalMarkerClick?: (agency: ExternalAgency, index: number) => void;
  onExternalMarkerHover?: (agency: ExternalAgency | null, index: number | null) => void;
  highlightedExternalIndex?: number | null;
  focusedExternalIndex?: number | null;
}
...
const EventMapComponent: React.FC<EventMapComponentProps> = ({
  ...
  className = '',
  externalAgencies = [],
  onExternalMarkerClick,
  onExternalMarkerHover,
  highlightedExternalIndex,
  focusedExternalIndex,
}) => {
  ...
  const externalIndexToMarkerRef = useRef<Map<number, L.Marker>>(new Map());
  ...
  const clearExternalMarkers = useCallback(() => {
    ...
    externalIndexToMarkerRef.current.clear();
  }, []);

  // Add external agency markers to map
  const addExternalMarkers = useCallback(
    (map: L.Map) => {
      clearExternalMarkers();

      externalAgencies.forEach((agency, index) => {
        const lat = Number(agency.latitude);
        const lng = Number(agency.longitude);
        if (!isValidCoord(lat) || !isValidCoord(lng)) return;

        const marker = L.marker([lat, lng], {
          icon: createExternalAgencyIcon(),
          zIndexOffset: -1000,
        }).addTo(map);

        const popupLines = [`<strong>${agency.name}</strong>`];
        if (agency.address) {
          popupLines.push(
            `<small>${agency.address}, ${agency.city}, ${agency.state} ${agency.zip}</small>`,
          );
        }
        if (agency.phone) {
          popupLines.push(`<small>${agency.phone}</small>`);
        }

        marker.bindPopup(`<div class="event-popup">${popupLines.join('')}</div>`, {
          maxWidth: 280,
          autoPan: false,
          closeButton: true,
        });

        externalIndexToMarkerRef.current.set(index, marker);

        marker.on('click', () => {
          onExternalMarkerClick?.(agency, index);
        });
        marker.on('mouseover', () => {
          onExternalMarkerHover?.(agency, index);
        });
        marker.on('mouseout', () => {
          onExternalMarkerHover?.(null, null);
        });

        externalMarkersRef.current.push(marker);
      });
    },
    [externalAgencies, onExternalMarkerClick, onExternalMarkerHover, clearExternalMarkers],
  );
      ...
      locationGroups.forEach((group) => {
        ...
        const popupContent =
          group.events.length === 1
            ? `<div class="event-popup">
                ...
                firstEvent.date
                  ? `<div class="event-popup-datetime">${
                      firstEvent.date
                    } • ${firstEvent.startTime || ''} - ${firstEvent.endTime || ''}</div>`
                  : ''
              }
						</div>`
            : `<div class="event-popup event-popup-multi">
							...
							<div class="event-popup-count">${group.events.length} events at this location</div>
							<ul class="event-popup-list">
								${group.events
                  .map(
                    (e) =>
                      `<li class="event-popup-item" data-index="${e.originalIndex}">
        ...
        marker.on('click', () => {
          if (group.events.length === 1) {
            onMarkerClick?.(group.events[0].event, group.events[0].originalIndex);
          }
        });
        ...
        marker.on('mouseover', () => {
          if (group.events.length === 1) {
            onMarkerHover?.(group.events[0].event, group.events[0].originalIndex);
          }
        });
    ...
    const geocodeZip = async () => {
      ...
      try {
        ...
        if (coords) {
          ...
          const lat = firstEvent.latitude || firstEvent.agencyLatitude;
          const lng = firstEvent.longitude || firstEvent.agencyLongitude;
          if (lat && lng) {
            setCenterCoords({ lat: Number(lat), lng: Number(lng) });
          } else {
            setHasError(true);
          }
        } else if (externalAgencies.length > 0) {
          const firstExternal = externalAgencies.find(
            (a) => isValidCoord(Number(a.latitude)) && isValidCoord(Number(a.longitude)),
          );
          if (firstExternal) {
            setCenterCoords({
              lat: Number(firstExternal.latitude),
              lng: Number(firstExternal.longitude),
            });
          } else {
            setHasError(true);
          }
  ...
  useEffect(() => {
    ...
    const markerTimeout = setTimeout(() => {
      if (mapInstanceRef.current) {
        // Add external markers first so event markers render on top
        addExternalMarkers(mapInstanceRef.current);
        addMarkers(mapInstanceRef.current);
      }
    }, 100);

    return () => clearTimeout(markerTimeout);
  }, [eventsWithCoords, isMapReady, addMarkers, addExternalMarkers]);
  ...
  useEffect(() => {
    const handlePopupItemClick = (e: MouseEvent) => {
      ...
      const eventData = eventsWithCoords.find((ev) => ev.originalIndex === originalIdx);
  ...
  // Highlight marker and open/close popup when highlightedIndex changes
  useEffect(() => {
    ...
    try {
      if (highlightedIndex === null || highlightedIndex === undefined) {
        markersRef.current.forEach((marker) => {
          ...
          if (iconElement) {
            const markerDiv = iconElement.querySelector('.marker-number');
            if (markerDiv) {
  ...
  // Highlight external marker and open/close popup when highlightedExternalIndex changes
  useEffect(() => {
    if (!isMapReady) return;

    try {
      // Remove highlight and close popup on all external markers
      externalIndexToMarkerRef.current.forEach((marker, idx) => {
        const iconElement = marker.getElement();
        if (iconElement) {
          const dot = iconElement.querySelector('.external-marker-dot');
          if (dot) {
            dot.classList.remove('highlighted');
          }
        }
        if (idx !== highlightedExternalIndex) {
          marker.closePopup();
        }
      });

      // Highlight and open popup on the target marker
      if (highlightedExternalIndex !== null && highlightedExternalIndex !== undefined) {
        const targetMarker = externalIndexToMarkerRef.current.get(highlightedExternalIndex);
        if (targetMarker) {
  ...
  }, [highlightedExternalIndex, isMapReady]);

  // Pan to marker when focusedIndex changes
  useEffect(() => {
    if (!isMapReady || focusedIndex === null || focusedIndex === undefined || focusedIndex < 0)
      return;
  ...
  // Pan to external agency marker when focusedExternalIndex changes
  useEffect(() => {
    if (
      !isMapReady ||
      focusedExternalIndex === null ||
      focusedExternalIndex === undefined ||
      focusedExternalIndex < 0
    )
      return;
    ...
    const timeoutId = setTimeout(() => {
      ...
      try {
        const marker = externalIndexToMarkerRef.current.get(focusedExternalIndex);
        if (marker) {
          const latLng = marker.getLatLng();
          map.setView(latLng, 14, { animate: true });
          marker.openPopup();
    ...
    return () => clearTimeout(timeoutId);
  }, [focusedExternalIndex, isMapReady]);
