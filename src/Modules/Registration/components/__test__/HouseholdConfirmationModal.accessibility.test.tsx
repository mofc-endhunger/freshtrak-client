...
describe('HouseholdConfirmationModal Accessibility', () => {
  ...
  describe('ARIA Attributes and Roles', () => {
    it('should have proper dialog role and ARIA attributes', () => {
      ...
      expect(dialog).toHaveAttribute('aria-labelledby', 'household-modal-title');
      expect(dialog).toHaveAttribute('aria-describedby', 'household-modal-description');
    ...
    it('should have proper title and description IDs', () => {
      ...
      const titles = screen.getAllByText('Use your household information?');
      expect(titles.length).toBeGreaterThan(0);
      const descriptions = screen.getAllByText(/We found household information on your account/);
      expect(descriptions.length).toBeGreaterThan(0);

      // Check that the visible elements with specific IDs exist
      const visibleTitle = screen.getByRole('dialog').querySelector('[id="household-modal-title"]');
  ...
  describe('Loading State Accessibility', () => {
    it('should have proper loading state ARIA attributes', () => {
      ...
      expect(loadingContainer).toHaveAttribute('aria-label', 'Processing registration');
    });

    it('should announce loading state to screen readers', () => {
      render(<HouseholdConfirmationModal {...defaultProps} isLoading={true} />);

      const loadingText = screen.getByText('Processing your registration...');
  ...
  describe('Error State Accessibility', () => {
    it('should have proper error state ARIA attributes', () => {
      render(<HouseholdConfirmationModal {...defaultProps} error="Test error message" />);
    ...
    it('should have proper error button descriptions', () => {
      ...
      expect(errorButton).toHaveAttribute('aria-describedby', 'error-description');

      const errorDescription = screen.getByText(/An error occurred during registration/);
  ...
  describe('Button Accessibility', () => {
    it('should have proper button descriptions', () => {
      ...
      expect(reviewButton).toHaveAttribute('aria-describedby', 'review-button-description');
    ...
    it('should have hidden descriptions for screen readers', () => {
      ...
      const reviewDescription = screen.getByText(/Navigate to the registration form/);
      const confirmDescription = screen.getByText(/Register immediately using your existing/);
    ...
    it('should disable confirm button when no household data', () => {
      render(<HouseholdConfirmationModal {...defaultProps} householdData={null} />);
  ...
  describe('Keyboard Navigation', () => {
    ...
    it('should handle Enter key on buttons', async () => {
      ...
      render(<HouseholdConfirmationModal {...defaultProps} onConfirm={onConfirm} />);
    ...
    it('should handle Space key on buttons', async () => {
      ...
      render(<HouseholdConfirmationModal {...defaultProps} onReview={onReview} />);
  ...
  describe('Screen Reader Support', () => {
    ...
    it('should announce changes in content', () => {
      const { rerender } = render(<HouseholdConfirmationModal {...defaultProps} />);

      // Initially shows household data
      expect(screen.getByText('Test Family')).toBeInTheDocument();

      // Change to loading state
      rerender(<HouseholdConfirmationModal {...defaultProps} isLoading={true} />);
      expect(screen.getByText('Processing your registration...')).toBeInTheDocument();

      // Change to error state
      rerender(<HouseholdConfirmationModal {...defaultProps} error="Test error" />);
  ...
  describe('Color Contrast and Visual Accessibility', () => {
    ...
    it('should have proper error state styling', () => {
      render(<HouseholdConfirmationModal {...defaultProps} error="Test error" />);
  ...
  describe('Semantic HTML Structure', () => {
    it('should use proper semantic elements', () => {
      ...
      const dialogTitle = headings.find(
        (h) =>
          h.textContent === 'Use your household information?' && h.id === 'household-modal-title',
      );
