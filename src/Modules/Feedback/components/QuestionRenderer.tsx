...
const QuestionRenderer: React.FC<QuestionRendererProps> = ({
  ...
  const handleChange = useCallback((v: string) => onChange({ answerValue: v }), [onChange]);

  const options = question.options ?? [];
  const sortedOptions =
    options.length > 0 ? [...options].sort((a, b) => a.display_order - b.display_order) : [];
        ...
        <div className="flex flex-col gap-2">
          {sortedOptions.map((opt) => (
            <Label
              ...
              className="flex items-center gap-2 cursor-pointer font-normal text-gray-700"
            >
              <Checkbox checked={selectedSet.has(opt.value)} onCheckedChange={() => toggle(opt)} />
              {opt.label}
            </Label>
          ))}
        </div>
  ...
  if (DROPDOWN_SELECT_ONE.has(type) && sortedOptions.length > 0) {
    return (
      <div className={cn('space-y-2', className)} data-testid={`question-${question.id}`}>
        {promptLabel}
        <Select value={answerStr || ''} onValueChange={(v) => handleChange(v ?? '')}>
          <SelectTrigger className="w-full min-w-0 overflow-hidden [&_[data-slot=select-value]]:min-w-0 [&_[data-slot=select-value]]:truncate [&_[data-slot=select-value]]:text-left">
            <SelectValue placeholder={getLocalizedString('placeholder_select_an_option')} />
          </SelectTrigger>
          <SelectContent className="z-[10002] bg-white max-w-[min(20rem,100vw)]">
            {sortedOptions.map((opt) => (
              <SelectItem key={opt.id} value={opt.value} className="hover:bg-gray-100 break-words">
                {opt.label}
              </SelectItem>
            ))}
          </SelectContent>
