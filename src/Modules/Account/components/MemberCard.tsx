...
import { getMemberCategory, getMemberCategoryLabel, getMemberInitials } from '../utils/memberUtils';
    ...
    <div className={`bg-white rounded-lg border border-gray-200 p-4 ${className}`}>
      <div className="flex justify-between items-start">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="font-noto-sans text-sm text-gray-500 mt-1">{categoryLabel}</p>
          {isFreshTrakUser && (
            <Badge className="bg-primary text-white text-xs mt-2">
              {localization.badge_freshtrak_user || 'FreshTrak User'}
            </Badge>
          )}
        </div>
        ...
        <div className="flex-shrink-0 ml-4">
          <div className="w-10 h-10 rounded-full bg-orange-200 flex items-center justify-center">
            <span className="font-noto-sans font-semibold text-sm text-orange-800">{initials}</span>
          </div>
        </div>
      </div>
    </div>
