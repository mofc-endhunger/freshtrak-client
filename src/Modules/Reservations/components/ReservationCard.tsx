...
const ReservationCard: React.FC<ReservationCardProps> = ({
  ...
  const showFeedbackButton =
    isPast && survey != null && (survey.status === 'in_progress' || survey.status === 'scheduled');
    ...
    <>
      ...
      >
        <CardContent className="p-3">
          ...
          {/* Date and Time */}
          <p
            className={`font-noto-sans text-sm mb-2 ${isPast ? 'text-gray-500' : 'text-gray-600'}`}
          >
          ...
          {isPast && status && (
            <div className="flex items-center justify-between gap-2">
              <Badge variant="outline" className={`${getStatusBadgeClass()} text-xs border`}>
