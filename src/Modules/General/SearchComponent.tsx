...
const SearchComponent = forwardRef<HTMLDivElement, SearchComponentProps>(
  (
    ...
    const [availability, setAvailability] = useState<string>(initialAvailability || 'next_7_days');
    const [reservations, setReservations] = useState<boolean>(initialReservations);
    const [showFilter, setShowFilter] = useState<boolean>(z_code !== undefined);
    ...
    return (
      <Fragment>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6 items-end w-full" ref={ref}>
          <div className="sm:col-span-2">
            <div className="flex flex-col gap-4">
              {showAddress && (
                <div className="w-full" data-testid="search-street">
              ...
              <input type="hidden" defaultValue={lat || ''} {...register('lat')} name="lat" />
              <input type="hidden" defaultValue={long || ''} {...register('long')} name="long" />
            </div>
          </div>
          ...
          <div className="w-full mt-2">
            {address.length === 0 && showAddress && (
              <p className="text-sm text-gray-600">
                Enter your address for customized results (Optional){' '}
              </p>
            )}
          </div>
        </div>
          ...
          {showFilter && (
            <FilterComponent
              ...
              reservations={{
                ...
                onChangeHandler: (e: { target: { value: boolean } }) => {
