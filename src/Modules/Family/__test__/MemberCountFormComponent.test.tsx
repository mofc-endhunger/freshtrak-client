...
describe("MemberCountFormComponent", () => {
	describe("Rendering", () => {
		test("should render without crashing", () => {
			render(<TestWrapper />);
			expect(
				screen.getByTestId("member-count-form-component")
			).toBeInTheDocument();
		});
		...
		test("should display the correct subtitle", () => {
			render(<TestWrapper />);
			expect(
				screen.getByText("Please tell us about your family members")
			).toBeInTheDocument();
		});
	...
	describe("Accessibility", () => {
		...
		test("should have proper screen reader labels", () => {
			...
			expect(screen.getByText("Decrease number of seniors")).toHaveClass(
				"sr-only"
			);
			expect(screen.getByText("Increase number of seniors")).toHaveClass(
				"sr-only"
			);
		});
		...
		test("should have proper form labels", () => {
			...
			expect(screen.getByText("Number of Seniors (60+)")).toHaveClass(
				"sr-only"
			);
			expect(screen.getByText("Number of Adults (18+)")).toHaveClass(
				"sr-only"
			);
