    ...
    if (!errors) {
      return (
        <div className="text-center space-y-4">
          ...
          <h2 className="text-2xl font-bold text-red-600">{localization.text_page_not_found}</h2>
          {error.message && (
        ...
        <div className="text-center space-y-4">
          ...
          <div className="space-y-2">
            {errors.map((error, index) => (
              <div key={`error-${index}`} className="text-gray-600 p-3 bg-orange-50 rounded-lg">
                <p className="font-semibold">
                  {localization.label_code_colon} {error.code}
                </p>
                <p>
                  {localization.label_message_colon} {error.message}
                </p>
              </div>
            ))}
          </div>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="flex justify-center">
            <Link to={RENDER_URL.ROOT_URL}>
              <Button data-testid="continue button" variant="highlight">
