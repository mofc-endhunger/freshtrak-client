...
import { HeaderComponentProps, PageType, MobileMenuSection } from './types/header.types';
...
const HeaderComponent: React.FC<HeaderComponentProps> = ({ shortHeader }) => {
   ...
   */
  const shouldShowBackground = (pageType: PageType, isScrolled: boolean): boolean => {
    // Main page, search results, and login page: transparent initially, background on scroll
    if (pageType === 'main' || pageType === 'search' || pageType === 'login') {
      return isScrolled;
    }
  ...
  const shouldShowBackgroundColor = shouldShowBackground(pageType, isScrolled);
  ...
  return (
    <Fragment>
      <nav
        ...
        style={{
          boxShadow: shouldShowBackgroundColor ? '0 4px 8px #28CE85' : '',
          overflow: 'visible',
        }}
        id="mainNav"
