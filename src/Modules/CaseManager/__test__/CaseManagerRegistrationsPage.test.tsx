...
jest.mock('../../Localization/LocalizationComponent', () => ({
  ...
  default: {
    ...
    cm_no_registrations_desc: 'Registrations you create on behalf of others will appear here.',
...
describe('CaseManagerRegistrationsPage', () => {
  ...
  describe('Error states', () => {
    ...
    it('shows generic load failure for non-403 errors', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText('Failed to load registrations. Please try again.'),
        ).toBeInTheDocument();
      });
    });
  ...
  describe('Empty state', () => {
    it('shows empty state when no registrations', async () => {
      ...
      await waitFor(() => {
        ...
        expect(
          screen.getByText('Registrations you create on behalf of others will appear here.'),
        ).toBeInTheDocument();
      });
    });
    ...
    it('shows Find an Event button in empty state', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByRole('button', { name: 'Find an Event' })).toBeInTheDocument();
      });
    });
    ...
    it('navigates to home when Find an Event is clicked', async () => {
      ...
      await waitFor(() => {
        fireEvent.click(screen.getByRole('button', { name: 'Find an Event' }));
      });
      expect(mockNavigate).toHaveBeenCalledWith('/');
    });
  });
  ...
  describe('Registrations table', () => {
    ...
    it('renders search input', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByPlaceholderText('Search by name, event, email or phone...'),
        ).toBeInTheDocument();
      });
    });
    ...
    it('filters registrations by search term', async () => {
      ...
      const searchInput = screen.getByPlaceholderText('Search by name, event, email or phone...');
