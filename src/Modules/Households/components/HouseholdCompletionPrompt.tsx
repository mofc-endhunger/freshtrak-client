...
const PROMPT_VARIANTS: Record<string, PromptConfig> = {
  banner: {
    ...
    description: 'Set up your household for personalized services and easier event registration.',
  ...
  card: {
    ...
    benefits: ['Track family members', 'Manage household preferences', 'Access family services'],
...
export const HouseholdCompletionPrompt: React.FC<HouseholdCompletionPromptProps> = ({
  onSetup,
  onDismiss,
  className = '',
  variant = 'banner',
}) => {
  const { shouldShowPrompt, markPromptShown } = useHouseholdSignUpIntegration();
  const { user } = useAuth();
          ...
          >
            <div className="flex items-start justify-between">
              <div className="flex items-start space-x-4">
                <div className="flex-shrink-0">{promptConfig.icon}</div>
                <div className="flex-1">
                  <h3 className="text-lg font-semibold text-blue-900 mb-2">{promptConfig.title}</h3>
                  <p className="text-blue-700 mb-4">{promptConfig.description}</p>
            ...
            <CardContent className="p-6">
              <div className="flex items-start space-x-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-green-700 mb-4">{promptConfig.description}</p>
        ...
        return (
          ...
          >
            <div className="flex items-start space-x-3">
              <div className="flex-shrink-0">{promptConfig.icon}</div>
              <div className="flex-1">
                <h4 className="font-semibold text-gray-900 text-sm mb-1">{promptConfig.title}</h4>
                <p className="text-gray-600 text-xs mb-3">{promptConfig.description}</p>
...
export const useHouseholdCompletionPrompts = () => {
  const { shouldShowPrompt, getSignUpState } = useHouseholdSignUpIntegration();
  const { user } = useAuth();
  ...
  const shouldShowBannerPrompt = (): boolean => {
    return user?.email
      ? shouldShowPrompt(user.email) && getPromptFrequency() === 'immediate'
      : false;
  };

  const shouldShowCardPrompt = (): boolean => {
    return user?.email ? shouldShowPrompt(user.email) && getPromptFrequency() === 'delayed' : false;
  };

  const shouldShowToastPrompt = (): boolean => {
    return user?.email
      ? shouldShowPrompt(user.email) && getPromptFrequency() === 'periodic'
      : false;
  };
