 ...
 */
const YourReservations: React.FC<YourReservationsProps> = ({ onReservationClick }) => {
  // State
  ...
  const reservationsApiService = useMemo(() => new ReservationsApiService(), []);
  ...
  const fetchReservations = useCallback(async () => {
    ...
    try {
      ...
      setError(localization.error_loading_reservations || 'Failed to load reservations');
    } finally {
      setIsLoading(false);
    }
    ...
    <div className="w-full">
      ...
      <div className="mb-4">
        <h2 className="font-noto-sans font-semibold text-lg text-gray-900">
          {localization.title_your_reservations || 'Your Reservations'}
        </h2>
        {upcomingReservations.length > 0 && (
          <p className="font-noto-sans text-sm text-gray-500 mt-1">
            {localization.text_you_have_event_coming_up || 'You have an event coming up.'}
          </p>
        )}
      </div>
      ...
      <LoadingCard
        isLoading={isLoading}
        operation={localization.loading_reservations || 'Loading reservations...'}
        className="w-full"
      >
        {/* Error State */}
        {error && (
        ...
        {!error && upcomingReservations.length === 0 && (
          <div className="text-center py-12 bg-gray-50 rounded-lg">
            ...
            <p className="font-noto-sans text-base text-gray-500">
              {localization.text_no_upcoming_events || 'No upcoming events found.'}
            </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {upcomingReservations.map((reservation) => (
              <ReservationCard
                ...
                variant="upcoming"
                onClick={onReservationClick ? handleReservationClick : undefined}
              />
            ))}
          </div>
        )}
