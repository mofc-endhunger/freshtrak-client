...
export const HouseholdSignUpWrapper: React.FC<HouseholdSignUpWrapperProps> = ({
  ...
  useEffect(() => {
    const checkHouseholdSetup = async () => {
      if (isAuthenticated && user && user.email && needsHouseholdSetup === true) {
      ...
      <>
        ...
        <AlertDialog open={showWarningDialog} onOpenChange={setShowWarningDialog}>
          <AlertDialogContent className="bg-white border border-gray-200 text-gray-900">
            <AlertDialogHeader>
              <AlertDialogTitle>{localization.household_warning_title}</AlertDialogTitle>
              <AlertDialogDescription>
                {localization.household_warning_description}
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
              <AlertDialogCancel onClick={handleWarningCancel} disabled={isProcessing}>
...
export const useHouseholdSignUpState = () => {
  ...
  const { getSignUpState, shouldShowPrompt, hasCompletedSetup, getHouseholdId } =
    useHouseholdSignUpIntegration();
