...
describe('EventNearByComponent', () => {
  ...
  it('toggles accordion items when clicked', () => {
    ...
    const secondAccordionButton = screen.getByText('Events for Next 7 days');
    ...
    expect(screen.queryByTestId('event-list-today')).not.toBeInTheDocument();
  });
  ...
  it('has proper accessibility attributes', () => {
    ...
    expect(firstButton).toHaveAttribute('aria-controls', 'accordion-content-0');

    // Check ARIA attributes for second accordion
    const secondButton = screen.getByText('Events for Next 7 days').closest('button');
    expect(secondButton).toHaveAttribute('aria-expanded', 'false');
    expect(secondButton).toHaveAttribute('aria-controls', 'accordion-content-1');
  });
