...
const HouseholdMembersSection: React.FC<HouseholdMembersSectionProps> = ({ householdData }) => {
  const navigate = useNavigate();

  // Filter out head of household - show only other members
  const otherMembers =
    householdData?.members?.filter((member: any) => member.is_head_of_household !== 1) || [];
      ...
      {otherMembers.length > 0 ? (
        ...
        <div className="bg-gray-50 rounded-lg p-6 text-center">
          <p className="font-noto-sans text-sm text-gray-500">
            {localization.text_no_other_members || 'No other household members added.'}
          </p>
        </div>
      )}
